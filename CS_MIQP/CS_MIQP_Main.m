function CS_MIQP_Main()
% CS_MIQP_Main()
% This function defines the structure for generating the MIQP problems
...
for varNum = varSizeRange
    for eqnNum = constraintSizeRange
        if eqnNum < varNum
            CS_MIQP_CreateLPFile(varNum, eqnNum, numOfProbs)
        end
    end
end
...
for folderNum = 1 : length(folderNames)
    ...
    if any(size(dir('*.lp' ),1))
        CS_MIQP_GenerateSubmissionFile(folderNames{folderNum}, 0, numOfSubmissionFiles);
    end
    cd ../..
end

cd(fullfile(pwd, 'L1_Problems\Instances'))
CS_MIQP_GenerateSubmissionFile('L1_Submission', 1);
mkdir('..\Results')
cd ../..
